	...
	<form action="editobject.php?edit=<?php echo $_GET['edit']; ?>" method="post">
		<div class="col-sm-8 blog-main">
			<div class="blog-post row">
		        ...
		        <div class="col-sm-8">
		        	<div class="addMedia">
						<button class="btn btn-sm btn-success add-picture" data-element="picture-element"><span class="glyphicon glyphicon-plus"></span> Photo</button>
		...
		<div class="col-sm-3 col-sm-offset-1 object-sidebar">
			...
			<div class="sidebar-module sidebar-module-inset">
				<h4>Edition</h4>
				<p><button class="btn btn-sm btn-success">Enregistrer</button> <a class="btn btn-sm btn-danger" href="object.php?id=<?php echo $_GET['edit']; ?>">Annuler</a></p>
			</div>
		</div>
	</form>
</div>
...
<script type="text/javascript">
	...
	$('.add-object').click(function(e)
	{
		...
		if ($(this).hasClass('upload-object'))
		{
				...
				$(this).before('<span class="uploading">Envoi...</span>');
				$(this).hide();
				var button = $(this);
				...
				$.ajax(
				{
					...
					success: function(data)
					{
						...
						$(button).before('<div class="alert alert-success">Fichier envoyé</div>')
					...
					{
						$(button).before('<div class="alert alert-danger">Erreur!</div>')
						$(button).html('<span class="glyphicon glyphicon-plus"></span>');
						$(button).show();
						$(button).removeClass('upload-object');
						$('.newFile-form').first().remove();
		...
		else
		{
			...
			$(this).html('T&eacute;l&eacute;charger le nouveau fichier');
		}
	});
	...
	$('.add-picture').click(function(e)
	{
		...
		if ($(this).hasClass('upload-picture'))
		{
				...
				$.ajax(
				{
					...
					success: function(data)
					{
						...
						//$(this).parent().parent().before('<div class="alert alert-success">Photo envoyée</div>')
					...
					error: function(jqXHR, textStatus, errorThrown)
					{
						$(this).parent().parent().before('<div class="alert alert-danger">Erreur!</div>')
						$(button).html('<span class="glyphicon glyphicon-plus"></span> Photo');
						$(button).show();
						$(button).removeClass('upload-picture');
						$('.new-picture-form').first().remove();
		...
		else
		{
			...
			$(this).html('T&eacute;l&eacute;charger la nouvelle image');
		}
	});
	...
	$('.add-video').click(function(e){
		...
		else
		{
			...
			$(this).html('T&eacute;l&eacute;charger la nouvelle video');
		}
	});

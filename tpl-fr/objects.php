<div class="container row">
	...
	<?php
		...
		if (!empty($objects))
		{
			...
			echo '<li '.(($pageNum <= 0)?'class="disabled"':'').'><a href="'.$url.(($pageNum <= 0)?'0':$pageNum-1).'">Precedent</a></li>';
		 	if ($pageNum >= 2)
			...
			echo '<li '.((count($objects) < OBJECTS_PER_PAGE)?'class="disabled"':'').'><a href="'.$url.((count($objects) < OBJECTS_PER_PAGE)?$pageNum:(($pageNum >= 2)?$pageNum+1:2)).'">Suivant</a></li>';
			echo '</ul>';
		}
		else
